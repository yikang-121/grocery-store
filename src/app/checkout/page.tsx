...
import {
  ...
  FaMobile, FaMapMarkerAlt, FaTruck, FaClock
} from "react-icons/fa";
import { Address } from "@/types/address"; // <- single source of truth
...
export default function CheckoutPage() {
  ...
  const validatePayment = () => {
    if (paymentMethod === "CARD") {
      if (!paymentDetails.cardholderName.trim()) return "Please enter cardholder name";
      if (!paymentDetails.cardNumber.trim()) return "Please enter card number";
      if (!paymentDetails.expiryDate.trim()) return "Please enter expiry date";
      if (!paymentDetails.cvv.trim()) return "Please enter CVV";
      const cardNum = paymentDetails.cardNumber.replace(/\s/g, "");
      if (cardNum.length < 13 || cardNum.length > 19) return "Invalid card number";
      const expiryRegex = /^(0[1-9]|1[0-2])\/\d{2}$/;
      if (!expiryRegex.test(paymentDetails.expiryDate)) return "Invalid expiry date (MM/YY)";
      if (paymentDetails.cvv.length < 3 || paymentDetails.cvv.length > 4) return "Invalid CVV";
    } else if (paymentMethod === "EWALLET") {
      if (!paymentDetails.eWalletType) return "Please select an e-wallet";
    }
    return null;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let paymentInfo: any = { method: paymentMethod };
      if (paymentMethod === "CARD") {
        paymentInfo = {
          ...paymentInfo,
        ...
        paymentInfo = { ...paymentInfo, eWalletType: paymentDetails.eWalletType };
      }

      // IMPORTANT: use normalized fields
      const orderData = {
        userId: user.id,
        items: cart.map(item => ({ productId: item.id, qty: item.quantity })),
        shippingAddressJson: JSON.stringify({
          name: selectedAddress.name,
          address: selectedAddress.address_line,       // <- street
          city: selectedAddress.city,
          state: selectedAddress.state,
          postal: selectedAddress.postal,    // <- postalCode
        }),
        notes: "",
        paymentMethod,
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                !showAddressStep ? 'bg-green-600 text-white' : 'bg-green-600 text-white'
              }`}>
                1
              </div>
            ...
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                showAddressStep ? 'bg-green-600 text-white' : 'bg-gray-300 text-gray-500'
              }`}>
                2
              </div>
          ...
          <>
           ...
           <p className="text-gray-600">
            ...
            <div className={`rounded-2xl shadow-lg p-6 mb-6 ${
              selectedAddress ? 'bg-white' : 'bg-gray-50'
            }`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <label className={`flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all ${
                  ...
                  <input
                    ...
                    className="mr-3"
                  />
                  <FaCreditCard className="mr-3 text-blue-600" size={20} />
                  <span className="font-medium">Credit/Debit Card</span>
                </label>

                <label className={`flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all ${
                  paymentMethod === "EWALLET" ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300'
                }`}>
                  <input
                    type="radio"
                    name="payment"
                    ...
                    className="mr-3"
                  />
                  <FaMobile className="mr-3 text-purple-600" size={20} />
                ...
                <label className={`flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all ${
                  ...
                  <input
                    ...
                    className="mr-3"
                  />
                  <FaUniversity className="mr-3 text-green-600" size={20} />
          ...
          <div>
            ...
            />
              ...
              {paymentMethod === "EWALLET" && (
                <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
                  <h4 className="font-semibold text-gray-800">Select E-Wallet</h4>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {["GrabPay", "Touch 'n Go", "Boost", "ShopeePay"].map((wallet) => (
                      <label key={wallet} className={`flex flex-col items-center p-3 rounded-lg border-2 cursor-pointer transition-all ${
                        paymentDetails.eWalletType === wallet ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300'
                      }`}>
              <input
                          type="radio"
                          name="ewallet"
                          value={wallet}
                          checked={paymentDetails.eWalletType === wallet}
                          onChange={(e) => setPaymentDetails({...paymentDetails, eWalletType: e.target.value})}
                          className="sr-only"
                        />
                  ...
                  <p className="text-sm text-gray-600">You'll be redirected to your selected e-wallet app to complete payment.</p>
                </div>
              )}

              {paymentMethod === "BANK_TRANSFER" && (
                <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
                  <h4 className="font-semibold text-gray-800">Bank Transfer Details</h4>
                  <div className="space-y-3">
                    <p className="text-sm text-gray-600">Transfer to the following bank account:</p>
                    <div className="bg-white p-4 rounded-lg border">
                      <div className="space-y-2">
                        <div><strong>Bank:</strong> Public Bank Berhad</div>
                        <div><strong>Account Number:</strong> 1234567890</div>
                        <div><strong>Account Name:</strong> VGrocery Sdn Bhd</div>
                        <div><strong>Amount:</strong> RM{total.toFixed(2)}</div>
                      </div>
                    </div>
                    <p className="text-xs text-gray-500">Please include your order reference in the transfer description.</p>
                  </div>
                </div>
              )}
